...
- **Deferido a la spec del financiero**: Fase 4 (precio máximo de compra por bisección sobre el motor + análisis de sensibilidad) y Fase 5 (one-pager + `BenchmarksPort` con entrada manual), más toda la capa web/UI (rutas `/calcular-dcf`, `/umbrales`, `/one-pager`; plantilla/JS/CSS). Todas surfacean números reales del DCF: se implementan al rellenar `construir_flujos`.
